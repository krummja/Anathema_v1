...
class WorldCreation(UIScreen):
    ...
    def generate(self):
        self.game.maps.generator.generate()
        self.game.maps.viewer.generate_standard_view(
    ...
    def get_current_view(self):
        ...
        if self.configuration['View'] == 'Biome':
            self.game.maps.viewer.generate_biome_view()
        if self.configuration['View'] == 'Rainfall':
            self.game.maps.viewer.generate_precipitation_view()
        if self.configuration['View'] == 'Temp':
            self.game.maps.viewer.generate_temperature_view()
        ...
        super().__init__(name = "FINALIZE WORLD", game = game, views = [
            RectView(
                ...
                subviews = [
                    ...
                    ButtonView(
                        "Done", callback = (lambda: print("Done")),
