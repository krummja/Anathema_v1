...
class PlayerManager(AbstractManager):
    ...
    def move(self, direction: Tuple[int, int]) -> None:
        ...
        if self.game.world.current_area.is_blocked(target_x, target_y):
            ...
            if self.game.world.current_area.is_interactable(target_x, target_y):
                ...
                self.action_queue.append(Action(self.entity, 'try_get_interactions', {'target': interactable, 'expect': []}))

            self.game.log.report(Message("The way is blocked!"))

        else:
            self.action_queue.append(Action(self.entity, 'try_move', {'target': direction}))
    ...
    def close(self, closable) -> None:
        ...
        if ((closable.has('Door') and closable['Door'].is_open) or
            ...
            self.action_queue.append(Action(self.entity, 'try_get_interactions', {'target': closable, 'expect': []}))

    def pickup(self) -> None:

        target = self.game.interaction_system.get(*self.position)
        self.action_queue.append(Action(self.entity, 'try_pickup', {'target': target, 'instigator': self.entity}))
